...
#     channel_id= request_body["channel_id"]
@router.get("/mychannels")
async def mychannels(request:Request,db_conn:DatabaseConnection.PooledMySQLConnection=Depends(get_conn)):
    request_body=await request.json()
    username=request_body["username"]
    token=request_body["token"]
    user=TokenInteraction.verify_token(token)
    if user:
         with db_conn.cursor(dictionary = True) as cursor:
            query="Select Channel.id,Channel.title,Channel.description from join_channel,channel where join_channel.channel_id=channel.id and studentusername=%s";
            cursor.execute(query,(username))
            ...
            if result:
                ...
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="User has no channels")
...
async def solve_quiz(request:Request,username :str = Depends(TokenInteraction.get_current_user),db_conn: DatabaseConnection.PooledMySQLConnection = Depends(get_conn)):
    ...
    with db_conn.cursor(dictionary = True) as cursor:
        ...
        if result:
            ...
            if result:
                release_conn(db_conn)
                raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail = "Already solved")
            ...
            release_conn(db_conn)
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail = "Join Channel First")
