...
const CreatePS = () => {
	...
	const HandleSubmit = (e) => {
		e.preventDefault();
		const error = {};
		if (!Deadline) error.Deadline = "ProblemSet Deadline Can't be empty";
		if (selectedQuestions.length == 0)
			error.selectedQuestions = "ProblemSet can't have no question!";
		let deadline = new Date(Deadline);
		let now = new Date();
		if (deadline < now) error.Deadline = "this time already passed!";
		seterror(error);
		Deadline = Deadline.replace("T", " ");
		if (Object.keys(error).length === 0) {
			console.log({  Deadline, selectedQuestions });
	...
	return (
		<div>
			<h1 className="text-center font-bold text-4xl mb-6">Create a ProblemSet!</h1>
			<form
				onSubmit={HandleSubmit}
				className="bg-amber-100 flex flex-col justify-center  items-center px-4 py-2  ">
				<div className="mb-4">
					<label htmlFor="Deadline" className=" block mx-4">
						Deadline
					</label>
					<input
						type="datetime-local"
				...
				<div>
					...
					{questions.map((question, index) => (
						<div
							key={index}
							className="mb-4 hover:border hover:border-black hover:rounded-xl px-4 py-2">
							<input
								type="checkbox"
